...
class Timeline {
    ...
    constructor(props) {
        ...
        this.userId = Cookie.get(ML_USER_ID_FALG); //SDK 自动创建的匿名用户的 objectId
        this.userSessionToken = Cookie.get(ML_USER_SESSION_TOKEN_FLAG); //SDK 自动创建的匿名用户的 sessionToken
        this.installation = Cookie.get(ML_INSTALLATION_FLAG);
        this.installationTime = Cookie.get(ML_INSTALLATION_TIME);
        ...
        if (!this.installation) {
            ...
            Cookie.set(ML_INSTALLATION_FLAG, mlInstallationFlag);
            Cookie.set(ML_INSTALLATION_TIME, mlInstallationTime);
        }
        ...
        if (!this.getUserId()) {
            ...
            this.anonymousUserFetch.then(res=> {
                if(res && res.objectId){
                    ...
                    Cookie.set(ML_USER_ID_FALG, res.objectId);
                    Cookie.set(ML_USER_SESSION_TOKEN_FLAG, res.sessionToken);
                }
            })
        }else{

...
module.exports = function(LC) {
   ...
   * — you should use the reference to <code>LC.history</code>
   ...
   */
  LC.History = function() {
    console.warn("LC.History is deprecated, please don't use it anymore.");
    this.handlers = [];
  ...
  LC.History.started = false;

  // Set up all inheritable **LC.History** properties and methods.
  _.extend(LC.History.prototype, LC.Events,
           /** @lends LC.History.prototype */ {
    ...
    getFragment: function(fragment, forcePushState) {
      if (LC._isNullOrUndefined(fragment)) {
    ...
    start: function(options) {
      if (LC.History.started) {
        throw new Error("LC.history has already been started");
      }
      LC.History.started = true;
      ...
      if (oldIE) {
        this.iframe = LC.$('<iframe src="javascript:0" tabindex="-1" />')
      ...
      if (this._hasPushState) {
        LC.$(window).bind('popstate', this.checkUrl);
      } else if (this._wantsHashChange &&
                 ('onhashchange' in window) &&
                 !oldIE) {
        LC.$(window).bind('hashchange', this.checkUrl);
      } else if (this._wantsHashChange) {
    ...
    // Disable LC.history, perhaps temporarily. Not useful in a real app,
    // but possibly useful for unit testing Routers.
    stop: function() {
      LC.$(window).unbind('popstate', this.checkUrl)

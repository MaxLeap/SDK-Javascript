...
module.exports = function(LC) {
   ...
   * @return {LC.Promise} A Promise that will be fulfilled with a
  ...
  var readAsync = function(file, type) {
    ...
    if (typeof(FileReader) === "undefined") {
      return LC.Promise.error(new LC.Error(
    ...
    reader.onloadend = function() {
      if (reader.readyState !== 2) {
        promise.reject(new LC.Error(-1, "Error reading file."));
        return;
      }
      ...
      if (!matches) {
        promise.reject(
            new LC.Error(-1, "Unable to interpret data URL: " + dataURL));
        return;
      }
  ...
  /**
   * A LC.File is a local representation of a file that is saved to the LC
   * cloud.
   ...
   *   var name = "photo.jpg";
   *   var parseFile = new LC.File(name, file);
   *   parseFile.save().then(function() {
   *     // The file has been saved to LC.
   *   }, function(error) {
   *     // The file either could not be read, or could not be saved to LC.
   *   });
   * }</pre>
   *
   * @class
   ...
   *     extension.
   */
  LC.File = function(name, data, type) {
    this._name = name;
    ...
    if (_.isArray(data)) {
      this._source = LC.Promise.as(encodeBase64(data), guessedType);
      this._metaData.size = data.length;
    } else if (data && data.base64) {
      // 也可以是 dataURL，内部已做兼容
      var dataBase64 = dataURItoBlob(data.base64, guessedType);
      this._source = LC.Promise.as(dataBase64, guessedType);
    } else if (data && data.blob) {
      this._source = LC.Promise.as(data.blob, guessedType);
    } else if (typeof(File) !== "undefined" && data instanceof File) {
      this._source = LC.Promise.as(data, guessedType);
    } else if(LC._isNode && global.Buffer.isBuffer(data)) {
      // use global.Buffer to prevent browserify pack Buffer module
      this._source = LC.Promise.as(data.toString('base64'), guessedType);
      this._metaData.size = data.length;
    } else if (_.isString(data)) {
      throw "Creating a LC.File from a String is not yet supported.";
    }
  };

  /**
   * Creates a fresh LC.File object with exists url for saving to AVOS Cloud.
   ...
   *     this is omitted, the content type will be inferred from the name's
   *     extension.
   * @return {LC.File} the file object
   */
  LC.File.withURL = function(name, url, metaData, type){
    if(!name || !url){
      throw "Please provide file name and url";
    }
    var file = new LC.File(name, null, type);
   ...
   * @return {LC.File} the file object
   */
  LC.File.createWithoutData = function(objectId){
    var file = new LC.File();
    file.id = objectId;
    return file;
  };
  ...
  LC.File.prototype = {
     ...
     * after you get the file from a LC.Object.
     * @return {String}
     */
    url: function() {
      return this._url;
    ...
    **/
    ...
    destroy: function(options){
      if(!this.id)
        return LC.Promise.error('The file id is not eixsts.')._thenRunCallbacks(options);
      var request = LC._request("files", null, this.id, 'DELETE');
     ...
     * @param {string} type
     * @return {LC.Promise} Resolved with the response
     * @private
     */
    _qiniuToken: function(type) {
      ...
      self._qiniu_key = key;
      return LC._request("qiniu", null, null, 'POST', data);
    },
     ...
     */
    /**
     * Saves the file to the LC cloud.
     * @param {Object} saveOptions
     * @param {UploadProgressCallback} [saveOptions.onProgress]
     * @param {Object} options A Backbone-style options object.
     * @return {LC.Promise} Promise that is resolved when the save finishes.
     */
    save: function() {
      var options = null;
      var saveOptions = {};
      ...
      if (!self._previousSave) {
        if(self._source) {
          ...
          self._previousSave = LC._request("files", self._name, null, 'POST', data).then(function(response) {

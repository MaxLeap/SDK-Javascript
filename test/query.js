// queries
var GameScore = LC.Object.extend("GameScore");
var Person = LC.Object.extend("Person");
var query = new LC.Query(GameScore);
describe("Queries",function(){
  describe("#Basic Queries",function(){
    it("should return Class Array",function(done){

      query = new LC.Query(GameScore);
  ...
  describe("#cloudQuery", function(){
    it("should return results.", function(done){
      LC.Query.doCloudQuery('select * from GameScore').then(function(result){
        debug(result);
    ...
    it("should return limited results.", function(done){
      LC.Query.doCloudQuery('select * from GameScore limit 10').then(function(result){
        debug(result);
        var results = result.results;
    ...
    it("should return count value.", function(done){
      LC.Query.doCloudQuery('select *,count(objectId) from GameScore limit 10').then(function(result){
        debug(result);
        var results = result.results;
        expect(results.length).to.be(10);
        expect(results[0].className).to.be("GameScore");
    ...
    it("should return count value too.", function(done){
      LC.Query.doCloudQuery('select *,count(objectId) from GameScore limit ?', [5]).then(function(result){
        debug(result);
        var results = result.results;
    ...
    it("should return syntax error.", function(done){
      LC.Query.doCloudQuery('select * GameScore limit 10').then(function(){
        throw "Shoud not be successfully.";
      },
      function(error){
        debug(error);
        expect(error).to.be.an(LC.Error);
        done();
      });
    });
  });
...
*/
  ...
  describe("destroyAll", function(){
    it("should be deleted", function(done){
      ...
      var promises = [];
      for(var i=0; i < 10; i++){
        var test = new LC.Object("deletedAll");
        test.set("number", i);
        promises.unshift(test.save());
      }
      // LC.Promise.when(promises).then(function() {done();});
      LC.Promise.when(promises).then(function(){
        var query = new LC.Query("deletedAll");
  ...
  describe("Compound Query",function(){
    it("should satisfy on 'or' conditions", function(done) {
      var lotsOfWins = new LC.Query("GameScore");
      lotsOfWins.greaterThan("score",150);

      var fewWins = new LC.Query("GameScore");
      fewWins.equalTo("cheatMode",true);

      var mainQuery = LC.Query.or(lotsOfWins, fewWins);
      mainQuery.find({
        success: function(results) {
    ...
    it("should satisfy on 'and' conditions", function(done) {
      ...
      var mainQuery = LC.Query.and(lotsOfWins, fewWins);
      mainQuery.find({
        success: function(results) {
          results.forEach(function(gs){
            expect(gs.get('score') > 150 && gs.get('cheatMode')).to.be.ok();

...
var GameScoreCollection = LC.Collection.extend({
  model: GameScore
});
...
describe('Objects', function(){
  ...
  describe('#Saving Objects', function(){
    it('should crate a Object', function(done){
      ...
      var point = new LC.GeoPoint({latitude: 80.01, longitude: -30.01});
    ...
    it('should create a User',function(done){
      var User = LC.Object.extend("User");
      var u = new User();
    ...
    it('should validate failed.', function(done){
      var TestObject = LC.Object.extend('TestObject', {
        validate: function (attrs, options){
          return new LC.Error(1, "test");
        }
      });
  ...
  describe("Relational Data",function(){
    ...
    it("should set relation ",function(done){
      ...
      myComment.save(null,{
        success:function(myComment){
          var query = new LC.Query(Comment);
    ...
    it("should save all successfully", function(done){
      ...
      query.first({
        success:function(result){
          ...
          var person = LC.Object.createWithoutData('Person', result.id);
          person.set('age', 30);
          LC.Object.saveAll([person],{
            success: function(){
              done();
            },
            error: function(err){
    ...
    it("should query relational data",function(done){
      ...
      query.first({
        success:function(result){
          debug(result instanceof Person);
          debug(result.get("likes") instanceof LC.Relation);
    ...
    it("should fetch when save when creating new object.", function(done){
      ...
      p.save().then(function(person) {
        expect(person.get('company')).to.be('leapcloud');
        done();
      }).catch(function(err) {
        throw err;
      });

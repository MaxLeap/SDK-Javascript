describe("sms", function() {
    ...
    it("should be verified", function(done){
      ...
      user.signUp(null, {
        success: function(user) {
          LC.User.requestMobilePhoneVerify(phoneNumber).then(function() {
            LC.User.logInWithMobilePhone(phoneNumber, '123456').then(function(user){
              debug(user);
              expect(user.get('mobilePhoneVerified')).to.be(false);
              LC.User.requestLoginSmsCode(phoneNumber).then(function(){
                throw "Unverfied.";
              }, function(err){
  ...
  describe("#requestSmsCode", function(){
    it("should send sms code.", function(done){
      LC.Cloud.requestSmsCode(phoneNumber).then(function(){
        LC.Cloud.requestSmsCode({

describe("App Searching", function() {
  describe("#LC.SearchSortBuilder", function(){
  ...
  describe('#LC.SearchQuery', function() {
   this.timeout(10000);
   it('should find something.', function(done) {
     var q = new LC.SearchQuery('GameScore');
     q.queryString('*');
     q.find().then(function(results) {
       expect(q.hits()).to.be.greaterThan(0);
       expect(results[0]).to.be.an(LC.Object);
       done();
     });
   });
   ...
   it('should sort by score with sort builder.', function(done) {
     var q = new LC.SearchQuery('GameScore');
     q.sortBy(new LC.SearchSortBuilder().descending('score'));
     q.queryString('*');
     q.find().then(function(results) {
       expect(q.hits()).to.be.greaterThan(0);
       console.dir(results);
